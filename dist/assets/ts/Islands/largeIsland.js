...
class MaxIsland {
    constructor() {
        ...
        this.getMaxIsland = (grid, x, y, mapId) => {
            if (x >= grid.length ||
                y >= grid[0].length ||
                x < 0 ||
                y < 0 ||
...
class Visualize {
    constructor() {
        ...
        this.checker = (row, col) => {
            if (row >= 0 && col >= 0 && row < rowSize && col < colSize)
                return true;
            return false;
        };
        this.traverse = (node, visited, count, endNode) => {
        ...
        this.visualizeMaxIsland = () => {
            time = 40 + (time - 1) * -2;
            gridContainer === null || gridContainer === void 0 ? void 0 : gridContainer.removeEventListener("mousedown", setWall);
            gridContainer === null || gridContainer === void 0 ? void 0 : gridContainer.removeEventListener("mouseover", setWall);
...
export const maxIsland = () => {
    ...
    setTimeout(() => {
        if (p1.length > 1) {
            let node = document.querySelector(`div[row='${final[0][1]}'][col='${final[0][2]}']`);
            node.style.backgroundColor = "yellow";
        }
        else {
            let node = document.querySelector(`div[row='${extensions2[0][1]}'][col='${extensions2[0][2]}']`);
            node.style.backgroundColor = "yellow";
        }
