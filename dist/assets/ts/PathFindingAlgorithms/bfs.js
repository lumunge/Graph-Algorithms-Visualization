import { rowSize, colSize, algorithmType, manualStart, bfsSteps, dijkstrasPath, startBtn, clearPathBtn, wallBtn, gridContainer, } from "../Grid/gridIndex";
...
const checkUpdateNode = (row, col, curr, checker, visited, count) => {
    if (row >= 0 && col >= 0 && row < rowSize && col < colSize) {
        ...
        if (wall == "1")
            return;
        let prow = curr.getAttribute("row");
        let pcol = curr.getAttribute("col");
        if (algorithmType.classList.contains("dijkstras")) {
            var cost = Math.min(Number(curr.getAttribute("cost")) + Number(node.getAttribute("weight")), Number(node.getAttribute("cost")));
        }
        else {
            var cost = Math.min(Number(curr.getAttribute("cost")) +
                Math.abs(Math.abs(prow - row) + Math.abs(pcol - col)), node.getAttribute("cost"));
        }
